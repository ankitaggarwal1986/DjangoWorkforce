# Generated by Django 3.2.8 on 2022-01-10 17:28

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='ProjectReport',
            fields=[
                ('Id', models.IntegerField(primary_key=True, serialize=False)),
                ('ProjectName', models.CharField(max_length=50)),
                ('ProjectOwner', models.CharField(max_length=20)),
                ('Task', models.CharField(max_length=50)),
                ('AssignTo', models.CharField(max_length=20)),
                ('EstimatedTime', models.TimeField()),
                ('ProductiveTime', models.TimeField()),
                ('Billing', models.DecimalField(decimal_places=2, max_digits=6)),
            ],
        ),
        migrations.CreateModel(
            name='TaskReport',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('Tasks', models.CharField(max_length=100)),
                ('TaskAssignTo', models.CharField(max_length=20)),
                ('EstimatedTime', models.TimeField()),
                ('ProductiveTime', models.TimeField()),
                ('Billing', models.DecimalField(decimal_places=2, max_digits=6)),
                ('Project_Id', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='performance.projectreport')),
            ],
        ),
    ]
